   ...
   const { addToCart } = useCart();  // useCart থেকে addToCart ফাংশন নিয়ে আসা
        ...
        useEffect(() => {
          const fetchProducts = async () => {
            const response = await fetch('https://shopfront-server.vercel.app/electronics');
      ...
      {products.map((product) => (
        ...
        >
          <div className="relative">
            ...
            />
            <div className="absolute top-3 left-3  text-gray text-xs font-bold px-2 py-1 rounded">
              ⭐ {product.rating}
            </div>
          </div>
